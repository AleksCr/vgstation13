...
/obj/item/weapon/tome/attack(mob/living/M as mob, mob/living/user as mob)
	M.add_attack_log(text("\[[time_stamp()]\] <font color='orange'>Has had the [name] used on him by [user.name] ([user.ckey])</font>"))
	user.add_attack_log(text("\[[time_stamp()]\] <font color='red'>Used [name] on [M.name] ([M.ckey])</font>"))
