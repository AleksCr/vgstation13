...
/obj/item/weapon/plastique/afterattack(atom/target as obj|turf, mob/user as mob, flag)
	...
	if(ismob(target))
		...
		user.add_attack_log("\[[time_stamp()]\] <font color='red'> [user.real_name] tried planting [name] on [target:real_name] ([target:ckey])</font>")
	...
	if(do_after(user, target, 50) && user.Adjacent(target))
		...
		if (ismob(target))
			var/mob/M=target
			target:add_attack_log("\[[time_stamp()]\]<font color='orange'> Had the [name] planted on them by [user.real_name] ([user.ckey])</font>")
