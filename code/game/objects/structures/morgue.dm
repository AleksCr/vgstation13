...
//		return
	...
	if(contents.len <= 0)
		...
		for (var/mob/living/M in inside)
			...
			//M.add_attack_log(\1)\2No point in this when the mob's about to be qdeleted
			//user.add_attack_log(\1)\2log_attack("\[[time_stamp()]\] <b>[user]/[user.ckey]</b> cremated <b>[M]/[M.ckey]</b>")
			M.death(1)
			M.ghostize()
			qdel(M)
			M = null
