...
/mob/living/silicon/robot/mommi/attack_hand(mob/user)
	...
	if(!istype(user, /mob/living/silicon))
		switch(user.a_intent)
			if(I_DISARM)
				user.add_attack_log(text("\[[time_stamp()]\] <font color='red'>Disarmed [src.name] ([src.ckey])</font>"))
				src.add_attack_log(text("\[[time_stamp()]\] <font color='orange'>Has been disarmed by [user.name] ([user.ckey])</font>"))
