...
/obj/item/weapon/reagent_containers/hypospray/attack(mob/M as mob, mob/user as mob)
	...
	if (reagents.total_volume)
		...
		if(M.reagents)
			...
			M.add_attack_log(text("\[[time_stamp()]\] <font color='orange'>Has been injected with [src.name] by [user.name] ([user.ckey]). Reagents: [contained]</font>"))
			user.add_attack_log(text("\[[time_stamp()]\] <font color='red'>Used the [src.name] to inject [M.name] ([M.key]). Reagents: [contained]</font>"))
